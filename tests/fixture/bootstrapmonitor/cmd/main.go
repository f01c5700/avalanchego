...
func main() {
	...
	initCmd := &cobra.Command{
		...
		RunE: func(*cobra.Command, []string) error {
			if err := checkArgs(namespace, podName, nodeContainerName); err != nil {
				return err
			}
			if len(dataDir) == 0 {
	...
	initCmd.PersistentFlags().StringVar(&dataDir, "data-dir", "", "The path of the data directory used for the bootstrap job")
	...
	waitCmd := &cobra.Command{
		...
		RunE: func(*cobra.Command, []string) error {
			...
			return bootstrapmonitor.WaitForCompletion(namespace, podName, nodeContainerName, pollInterval)
		},
	}
...
func checkArgs(namespace string, podName string, nodeContainerName string) error {
	if len(namespace) == 0 {

...
# The published name should be 'avaplatform/bootstrap-monitor', but to avoid unintentional
# pushes it is defaulted to 'bootstrap-monitor' (without a repo or registry name) which can
# only be used to create local images.
export DOCKER_IMAGE=${DOCKER_IMAGE:-"bootstrap-monitor"}

# Use the build script for the bootstrap monitor
export EXTRA_BUILD_ARGS="--build-arg BUILD_SCRIPT=build_bootstrap_monitor.sh"

# Skip building the race image
export SKIP_BUILD_RACE=1

# Reuse the avalanchego build script for convenience. The image will have a CMD of
# "./avalanchego", so to run the bootstrap monitor will need to specify
# ./bootstrap-monitor".
#
# TODO(marun) Figure out how to set the CMD for a multi-arch image.
bash -x "${AVALANCHE_PATH}"/scripts/build_image.sh
